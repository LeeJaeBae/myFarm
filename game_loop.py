...
# load  images
spritesheet = pygame.image.load(path.join(images_folder, 'character.png'))
spritesheet_rev = pygame.image.load(path.join(images_folder, 'character_rev.png'))
...
sheep_img = pygame.image.load(path.join(images_folder, "sheep_eat.png"))
...
class Player(pygame.sprite.Sprite):
    def __init__(self, position):
        ...
        self.sheet_rev = spritesheet_rev
        self.sheet.set_clip(pygame.Rect(0, 0, 55, 72))
        self.image = self.sheet.subsurface(self.sheet.get_clip())
        self.image_rev = self.sheet_rev.subsurface(self.sheet_rev.get_clip())
        self.rect = self.image.get_rect()
        self.rect_rev = self.image_rev.get_rect()
...
class Actionzone_farming(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.image.fill(Color((255, 255, 0, 100)))
        self.rect = self.image.get_rect()
        self.rect.x = x  # 275
        self.rect.y = y  # 475
...
class Actionzone_ranching(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.image.fill(Color(255, 255, 0, 50))
        self.rect = self.image.get_rect()
        self.rect.x = x  # 275
        self.rect.y = y  # 475
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if player.rect.colliderect(
                action_zone_ranching_1.rect) and inventory.haypenny >= 32 and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                player.buy_llama()
                # player.move_llama()
        ...
        if player.rect.colliderect(action_zone_ranching_3.rect) and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                player.go_fish()
        if player.rect.colliderect(
                action_zone_shop.rect) and inventory.haypenny >= 100 and event.type == pygame.KEYDOWN:
